...
/*
 NEED TO RE-WRITE WITH THE IDEA THAT MULTIPLE TRACKS CAN BE ATTRIBUTED TO A GIVEN DECAY (USE THE DECAYINDEX VARIABLE AS A MASTER LOOP WITHIN THE EVENT)
 */
...
namespace larlite {
    ...
    bool FindMuonTracks::analyze(storage_manager* storage){
        ...
        for(size_t track_index=0; track_index < ass_info.size(); ++track_index) {
            ...
            for(size_t itrack = 0;itrack<TrackListInfo.size();itrack++){
                if(_Run == TrackListInfo[itrack][0] && _SubRun == TrackListInfo[itrack][1] && _Event == TrackListInfo[itrack][2] && (int)one_track.ID() == (int)TrackListInfo[itrack][5]){
            ...
            std::cout << "about to reset containers" << std::endl;
            for(int iPlane = 0;iPlane<3;iPlane++){
                gGausHits[iPlane]->SetMarkerColor(3);
            ...
            std::cout << "containers reset" << std::endl;
            ...
            std::cout << "track meta data acquired" << std::endl;
    ...
    void FindMuonTracks::DrawTrack(){
        ...
        gGausHits[0]->SetTitle(Form("%05d_%03d_%05d_%02d_U",(int)_Run,(int)_SubRun,(int)_Event,(int)TrackID));
        gGausHits[1]->SetTitle(Form("%05d_%03d_%05d_%02d_V",(int)_Run,(int)_SubRun,(int)_Event,(int)TrackID));
        gGausHits[2]->SetTitle(Form("%05d_%03d_%05d_%02d_Y",(int)_Run,(int)_SubRun,(int)_Event,(int)TrackID));
        ...
        cWireSignal->Update();
        cWireSignal->SaveAs(Form("plot/Tracks_%05d_%03d_%05d_%02d.pdf",(int)_Run,(int)_SubRun,(int)_Event,(int)TrackID));
    }
